...
export default async function handler(req, res) {
  ...
  const { fullname, email, phone, country, company, message, type } = req.body;
  ...
  // --- DYNAMIC EMAIL CONTENT LOGIC ---
  let emailSubject = "";
  let emailBody = "";

  if (type === 'NEWSLETTER') {
    emailSubject = `[Newsletter] New Subscription: ${email}`;
    emailBody = `You have a new newsletter subscriber!\n\nEmail: ${email}`;
  } 
  else if (type === 'LISTING REQUEST') {
    emailSubject = `[Broker] Listing Request: ${company}`;
    emailBody = `A new broker wants to list with us:\n\n` +
                `Broker Name: ${fullname}\n` +
    ...
    // Default to ENQUIRY
    emailSubject = `[Enquiry] New Lead: ${fullname}`;
    emailBody = `A new lead has submitted an enquiry:\n\n` +
                `Name: ${fullname}\n` +
                `Email: ${email}\n` +
                `Phone: ${phone}\n` +
                `Country: ${country}\n` +
